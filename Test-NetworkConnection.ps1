...
#Loop Test-Connection as many times as entered into form
    ...
    $y=1
        For ($i=$pingsToSend; $i -gt 0; $i--) {
            if ( (Test-NetConnection -ComputerName $siteToPing).PingSucceeded){
            $x = Test-NetConnection -ComputerName $siteToPing | Select ComputerName, @{Name="RoundTrip";expression={$_.PingReplyDetails.RoundtripTime}}
